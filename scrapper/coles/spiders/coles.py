...
class ColesSpider(scrapy.Spider):
    ...
    def get_search_url(self, response):
        ...
        order_by, begin_index, catalog_id, category_id = self.extract_search_parameters(search_dirty_parameters)
        search_url = self.generate_search_url(order_by, begin_index, catalog_id, category_id)
        return search_url 
    ...
    def extract_search_parameters(self, search_dirty_parameters):
        ...
        catalog_id = re.findall(regex, search_dirty_parameters)[6]  or ""
        category_id = re.findall(regex, search_dirty_parameters)[7] or ""
    ...
    def get_products_url(self, response):
        ...
        if has_next:
            ...
            if len(search_dirty_parameters_list) > 0:
                ...
                order_by, begin_index, catalog_id, category_id = self.extract_search_parameters(search_dirty_parameters)
                search_url = self.generate_search_url(order_by, int(begin_index) + self.SEARCH_TOTAL_VALUES, catalog_id, category_id)
    ...
    def get_price_per_region_url(self, region, product):
        try:
            ...
            params = "redirectUrl={0}&suburbPostCodeId={1}&serviceType={2}&externalffmcId={3}&state={4}&score={5}&collectionpoint={6}&suburb={7}&zoneid={8}&postcode={9}&longitude={10}&latitude={11}&storeId=10601".format(product['redirect_url'], region['id'],
